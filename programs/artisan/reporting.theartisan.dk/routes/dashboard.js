...
router.get('/', async (req, res) => {
  ...
  let distributions = {};
  try {
    distributions = await loadDistributions();
  } catch (err) {
    error = 'Could not load distribution settings.';
    console.error('Distribution settings error:', err.message);
  }
  ...
  try {
    ...
    const curr = await categorizeBillLines(billLines, accountMap);
    groups      = curr.groups;
    uncategorized = curr.uncategorized;

    const prev = await categorizeBillLines(prevBillLines, accountMap);
    ...
    for (let idx = 0; idx < histKeys.length; idx += 1) {
      const hKey = histKeys[idx];
      const hGrouped = await categorizeBillLines(histResults[idx], accountMap);
    ...
    const labourAlloc     = await loadAllocations(currentMonthKey);
    const prevLabourAlloc = await loadAllocations(previousMonthKey);
    ...
    const fixedAlloc        = await loadFixedAlloc(currentMonthKey);
    const prevFixedAlloc    = await loadFixedAlloc(previousMonthKey);
