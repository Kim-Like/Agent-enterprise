...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const { groups } = await categorizeBillLines(billLines, accountMap);
  ...
  let alloc = { tabs: { cafe: 90, events: 0, b2b: 10, webshop: 0 } };
  try {
    alloc = await loadFixedAlloc(monthKey);
  } catch (err) {
    error = error || 'Could not load fixed cost allocation settings.';
    console.error('Fixed allocations load error:', err.message);
  }
