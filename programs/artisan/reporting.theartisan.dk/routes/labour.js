...
const { loadAllocations, DEFAULT_TABS, DEFAULT_ROLES } = require('../utils/labourService');
...
router.get('/', async (req, res) => {
  ...
  let allocations = { tabs: DEFAULT_TABS, roles: DEFAULT_ROLES, deduction: 0 };
  try {
    allocations = await loadAllocations(monthKey);
  } catch (err) {
    error = error || 'Could not load labour allocation settings.';
    console.error('Labour allocations load error:', err.message);
  }
  res.render('labour', {
    monthKey,
