...
description: PM skill for Claude Code, Codex, Cursor, and Windsurf. Diagnoses SaaS metrics, critiques PRDs, plans roadmaps, runs discovery, coaches PM career transitions, and pressure-tests AI product decisions. In Agent Enterprise, use this skill whenever the user is working on portfolio governance, `agents/registry.json`, `programs/registry.json`, agent or task enablement, remote-estate operating decisions, or roadmap and PRD critique for the control plane.
---
...
## Agent Enterprise Overlay

This installed copy is tuned for the `Agent Enterprise` repo.

- If the user mentions `Agent Enterprise`, the control plane, `package.json`, `agents/registry.json`, `programs/registry.json`, program lanes, master lanes, activation order, Lavpris, Theartis, cPanel estate, or remote SSH-first operations, read `knowledge/agent-enterprise-context.md` first.
- Treat repo-root `package.json`, `agents/registry.json`, and `programs/registry.json` as authoritative. Treat older `AI-Enterprise` wording in docs and READMEs as legacy unless the user explicitly asks for historical context.
- For Lavpris or Theartis operating decisions, also read `docs/lavpris-ssh-first-operations.md` and `docs/theartis-cpanel-estate-operations.md` from the repo root before making recommendations.
- Use `Agent Enterprise` as the default project name in outputs. Only use `AI-Enterprise` when quoting or discussing legacy material.
- For control-plane PRDs, roadmaps, and prioritization, keep the current runtime boundaries front and center: single-process control plane, registry-first planning, and explicit `enabled` / `ready` / `blocked` / `held` states.
...
| User Intent | Framework | Load |
|---|---|---|
| "Agent Enterprise" / "control plane" / "portfolio governance" / "program sequencing" | Portfolio sequencing + tradeoff critique | `knowledge/agent-enterprise-context.md` + `knowledge/strategy-positioning.md` |
| "agents/registry.json" / "activation order" / "enablement state" / "blocked lane" / "master lane" | Dependency governance + activation sequencing | `knowledge/agent-enterprise-context.md` + `knowledge/strategy-positioning.md` |
| "programs/registry.json" / "program classification" / "active remote hold stub" | Portfolio classification + prioritization | `knowledge/agent-enterprise-context.md` + `knowledge/strategy-positioning.md` |
| "Lavpris" / "Lavprishjemmeside" / "Theartis" / "cPanel estate" / "ssh-first operations" / "remote estate" | Operational decision framing + risk tradeoffs | `knowledge/agent-enterprise-context.md` + `knowledge/strategy-positioning.md` |
| "control plane PRD" / "Agent Enterprise roadmap" / "registry-driven planning" / "problem framing for Agent Enterprise" | PRD and roadmap critique under current runtime constraints | `knowledge/agent-enterprise-context.md` + `knowledge/artifacts-delivery.md` |
